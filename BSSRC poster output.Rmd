...
stand_8_shp <- SHPFILE1 %>% filter(stand_id == 8)   #1  #8  #9*
stand_5_phos <- plot_phos %>% filter(stand_id.x == 5)   #3  #5  #4*
...
### # geom_sf_label(data = SHPFILE1, aes(label = A), size=3) +
 geom_point(data = stand_5_phos, shape= 17, aes(x = x_lat, y = y_long, 
                                      ...
                                      size = P, color = P)) +
  #  scale_size_continuous(limits=c(0, 6), breaks=seq(0, 6, by=2)) +
    #scale_alpha_continuous(limits=c(34, 48), breaks=seq(34, 48, by=2)) +
    #scale_color_continuous(guide = "legend", limits=c(0, 6), breaks=seq(0, 6, by=2), type = "viridis") +
    theme_minimal(base_size = 8))



stand_1_shp <- SHPFILE1 %>% filter(stand_id == 1)   #1  #8  #9*
stand_3_phos <- plot_phos %>% filter(stand_id.x == 3)   #3  #5  #4*
...
#this makes them in a row
prow <- plot_grid(
  p3 + theme(legend.position="none", axis.text.x = element_text(angle = 90)) +ggtitle("Phosphorus gradient in stands")+ theme(plot.title = element_text(size=20)),
