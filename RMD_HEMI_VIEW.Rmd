...
varibs <- dat1 %>% summarise(plot_id, openness, lai_2000_lin, ppfd_total_under_per_day_m_jor_mol_m2day, gap_fraction, row, stand, n_seedlings) %>% distinct()

dat2 <- dat1%>% dplyr::select(plot_id, species_code, dried_weight_g) %>%
  pivot_wider(names_from = species_code, values_from = dried_weight_g, values_fn = sum, values_fill = 0)
...
dat1test <- dat1 %>% filter(!plot_id=="9-6-K") 
...
dat146 <- dat146 %>% column_to_rownames("plot_id")


nmdsE <- metaMDS(comm = dat146,
             autotransform = FALSE,
             distance = "bray",
             engine = "monoMDS",
             k = 3,
...
ef <- envfit(nmdsE ~ lai_2000_lin, trts, permutations = 99999, choices = 1:3, na.rm=T, iterations = 10000)
ef

ef1 <- envfit(nmdsE ~ stand, trts, permutations = 999, choices = 1:3, na.rm = T, iterations= 10000)
ef1



ef2 <- envfit(nmdsE ~ n_seedlings, trts, permutations = 999, choices = 1:3, na.rm = T, iterations= 10000)

ef2

ef3 <- envfit(nmdsE ~ seed_trt, trts, permutations = 99999, choices = 1:3, na.rm = T, iterations= 10000)

ef3

ef4 <- envfit(nmdsE ~ biomass_sum, trts, permutations = 99999, choices = 1:3, na.rm = T, iterations= 10000)
...
ef <- envfit(ord ~ gap_fraction, vinput, choices = 1:3, na.rm=T)
ordiplot3d(nmdsD, col = "red", ax.col= "black", pch = 18, envfit = ef)
...
perm_weight %>% filter(MUEX >= 0.01)



#adonis2(perm_weight ~ stand, data = perm_ef, permutations = 999999)


#adonis2(perm_weight ~ standbyrow, data= perm_ef, permutations = 999999)

#adonis2(perm_weight ~ seed_trt, data= trts, permutations = 999999)

#adonis2(perm_weight ~ row, data = trts, 999999)
