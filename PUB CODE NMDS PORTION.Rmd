...
r2d2<- r2d2 + labs(title = "Dried Weight of Seeded Species in Seeded Stands",fill = "Species Code") + scale_fill_manual(values=cbPalette)

r2d2
r2<-r2 + scale_fill_manual(values = cbPalette)
r2
...
varibs<- varibs %>% left_join(dat1_summary_noPIPA, by = "plot_id")
varibs$trt <- ifelse(varibs$stand %in% 6:9, "Seeded", "Unseeded") %>% as.factor()
```
...
spp.vect <- envfit(nmdsE, input_dat.no.rare, permutations = 999, choices = c(1:3)) # changed from c(1:2) 
...
spp.ord.sig.seed3 <- filter(spp.ord.sig, Species == c("ARBE"))
...
#changed GN  #names(varibs)[c(2,6)] <- c("Number of Seedlings", "LAI 2000", "Gap Fraction")
  names(varibs)[c(2,6,10)] <- c("Number of Seedlings", "Gap Fraction","Total Biomass")
  env.vect <- envfit(nmdsE, varibs[,c(2,6,10)], permutations = 999, na.rm = T, choices= 1:3)
}
if(input_dat_choice == 2){
...
env.ord.sig<-subset(env.ord,pvals<0.4)
...
#------------------------Second Plot view---------------------------------------
# The following has been altered to show "total biomass" as a vector in NMDS axis 2 and 3
p2 <- ggplot(data = plot.dat, aes(x=MDS3, y=MDS2)) + theme_bw() + 
  geom_point(data = plot.dat, aes(fill = Trt.Row), alpha=0.65, size = 3, pch = 21) + 
  geom_segment(data=spp.ord.sig,aes(x=0,xend=NMDS3,y=0,yend=NMDS2),
               arrow = arrow(length = unit(0.3, "cm")),colour="grey64", alpha=0.8, size = 0.8,
               inherit.aes=FALSE) + 
  geom_segment(data=spp.ord.sig.seed,aes(x=0,xend=NMDS3,y=0,yend=NMDS2), #seeded spp
               arrow = arrow(length = unit(0.3, "cm")),colour="red4", size = 1.1,
              inherit.aes=FALSE) +
  geom_segment(data=spp.ord.sig.seed1,aes(x=0,xend=NMDS3,y=0,yend=NMDS2), #seeded spp
               arrow = arrow(length = unit(0.3, "cm")),colour="red4", size = 1.1,
              inherit.aes=FALSE) +
  geom_segment(data=spp.ord.sig.seed2,aes(x=0,xend=NMDS3,y=0,yend=NMDS2), #seeded spp
               arrow = arrow(length = unit(0.3, "cm")),colour="red4", size = 1.1,
              inherit.aes=FALSE) +
    geom_segment(data=spp.ord.sig.seed3,aes(x=0,xend=NMDS3,y=0,yend=NMDS2), #seeded spp
               arrow = arrow(length = unit(0.3, "cm")),colour="red4", size = 1.1,
              inherit.aes=FALSE) +
  geom_segment(data=env.ord.sig,aes(x=0,xend=NMDS3,y=0,yend=NMDS2),
               arrow = arrow(length = unit(0.3, "cm")),colour="black", size = 0.8,
               inherit.aes=FALSE) +   
 ...
 geom_text_repel(data=spp.ord.sig,max.iter = 1000,max.overlaps = 6, max.time = 16, aes(x=NMDS3,y=NMDS2,label=Species),size=2.2, color = "black",    inherit.aes = F, hjust = ifelse(spp.ord.sig$NMDS3 < 0, 1.2, 1)) +
  geom_text_repel(data=env.ord.sig, max.overlaps = Inf, aes(x=NMDS3,y=NMDS2,label=Variables),size=5, color = "black", 
                  inherit.aes = F, hjust = ifelse(env.ord.sig$NMDS3 < 0, 1.2, 1.4)) +
  scale_color_brewer(palette = "Spectral", name = "95% Confidence Ellipse", 
                     labels = c("Seeded + Leave", "Seeded + Take Out", 
                                "Unseeded + Leave", "Unseeded + Take Out")) +
...
p3 <- ggplot(data = plot.dat, aes(x=MDS1, y=MDS2)) + theme_bw() + 
  geom_point(data = plot.dat, aes(fill = Trt.Row), alpha=0.65, size = 3, pch = 21) + 
  geom_segment(data=spp.ord.sig,aes(x=0,xend=NMDS1,y=0,yend=NMDS2),
               arrow = arrow(length = unit(0.3, "cm")),colour="grey64", alpha=0.8, size = 0.8,
               inherit.aes=FALSE) + 
  ...
  geom_segment(data=spp.ord.sig.seed2,aes(x=0,xend=NMDS1,y=0,yend=NMDS2), #seeded spp
               arrow = arrow(length = unit(0.3, "cm")),colour="red4", size = 1.1,
              inherit.aes=FALSE) +
    geom_segment(data=spp.ord.sig.seed3,aes(x=0,xend=NMDS1,y=0,yend=NMDS2), #seeded spp
               arrow = arrow(length = unit(0.3, "cm")),colour="red4", size = 1.1,
              inherit.aes=FALSE) +
  geom_segment(data=env.ord.sig,aes(x=0,xend=NMDS1,y=0,yend=NMDS2),
               arrow = arrow(length = unit(0.3, "cm")),colour="black", size = 0.8,
  ...
  #GN # geom_path(data = df.ell, aes(x=MDS1, y=MDS2, color = as.factor(Clusters), 
   ...
   #GN#         size = 1.5) + 
  #GN?#      theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + #GN hashtaged this out to allow it to render. Unknown issue
 geom_text_repel(data=spp.ord.sig,max.iter = 1000,max.overlaps = 6, max.time = 16, aes(x=NMDS1,y=NMDS2,label=Species),size=2.3, color = "black",    inherit.aes = F, hjust = ifelse(spp.ord.sig$NMDS1 < 0, 1.2, 1)) +
...
ggsave(filename = "ord2pub.png", plot = p2, width = 23, height = 13, units = "cm")
ggsave(filename = "ord3pub.png", plot = p3, width = 19, height = 12, units = "cm")
#------------------------Second Plot view---------------------------------------
```
...
# trying to have new things in the vectors of NMDS 

seededwt<- dplyr::filter(varibs, trt== "seeded")
seededwt


average_weights <- varibs %>%
  group_by(trt) %>%
  summarise(mean_dried_weight = mean(biomass_sum, na.rm = TRUE),
    sd_dried_weight = sd(biomass_sum, na.rm = TRUE)
  )

average_weights2 <- dat1_summary %>%
  summarise(mean_dried_weight = mean(biomass_sum, na.rm = TRUE))


mean(average_weights$mean_dried_weight)


checking<- dat1_long %>% filter(plot_id == "10-4-I")
sum(checking$biomass)
