...
p3 <- ggplot(data = plot.dat, aes(x=MDS1, y=MDS2)) + theme_bw() + 
  ...
  #GN # geom_path(data = df.ell, aes(x=MDS1, y=MDS2, color = as.factor(Clusters), 
   ...
   #GN#         size = 1.5) + 
  #GN?#      theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + #GN hashtaged this out to allow it to render. Unknown issue
 geom_text_repel(data=spp.ord.sig,max.iter = 1000,max.overlaps = 6, max.time = 16, aes(x=NMDS1,y=NMDS2,label=Species),size=2.2, color = "black",    inherit.aes = F, hjust = ifelse(spp.ord.sig$NMDS1 < 0, 1.2, 1)) +
...
ggsave(filename = "ord3pub.png", plot = p3, width = 23, height = 13, units = "cm")
#------------------------Second Plot view---------------------------------------
```
