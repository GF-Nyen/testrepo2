...
#changed GN  #names(varibs)[c(2,6)] <- c("Number of Seedlings", "LAI 2000", "Gap Fraction")
  names(varibs)[c(2,4,6,9)] <- c("Number of Seedlings", "Gap Fraction","Total Biomass")
...
```
 ...
 Gabe Messed with this chunk below to get the spp vector - biomass on the ord
```{r}
#--------Extract significant environmental vectors for plotting-----------------
...
if(input_dat_choice == 1){
#changed GN  #names(varibs)[c(2,6)] <- c("Number of Seedlings", "LAI 2000", "Gap Fraction")
   names(varibs)[c(2,4,6,9)] <- c("Number of Seedlings", "Solar Radiation", "Gap Fraction","Total Biomass")
  env.vect <- envfit(nmdsE, varibs[,c(2,4,6,9)], permutations = 999, na.rm = T)
}
if(input_dat_choice == 2){
  names(varibs.stand.row)[c(3,4,9,11)] <- c("Number of Seedlings", "Openness", "Total Biomass", "Seeded Biomass")
  env.vect <- envfit(nmdsE, varibs.stand.row[,c(3,4,9,11)], permutations = 999, na.rm = T)
...
binary_TMB <- glmmTMB(
  binary ~ biomass_sum + (1 | stand/row),
  family = binomial(link = "cloglog"),
  data = trtsNP_A
)
summary(binary_TMB)
df.residual(binary_TMB)

binary_TMB2 <- glmmTMB(
  binary ~ seed_trt -1 + (1 | stand/row),
  family = binomial(link = "cloglog"),
  data = trtsNP_A
)
summary(binary_TMB2)
df.residual(binary_TMB2)

compare_performance(binary_TMB,binary_TMB2)
 ### fit_binary has a significant factor of biomass_sum

compare_performance(binary_TMB,fit_binary)

compare_performance(binary_TMB,TMB2)

model_performance(TMB2)
...
```{r "final models for the publication table"}
binary_TMB <- glmmTMB(
  binary ~ biomass_sum + (1 | stand/row),
  family = binomial(link = "cloglog"),
  data = trtsNP_A
...
TMB2 <- glmmTMB(n ~ seed_trt -1 +  ppfd_total_under_per_day_m_jor_mol_m2day * biomass_sum + (1 | stand/row),    data = subset(trtsNP_A, n > 0.01), 
                     family  = truncated_nbinom2(link = "log"))
TMB2
summary(TMB2)
df.residual(TMB2)
check_model(TMB2, check = "reqq")

check_model(TMB2, check = "binned_residuals")

compare_performance(mod2,TMB2)
...
residuals(mod2)
  check_model(mod2)
  mod2subset<- subset(trtsNP_A, n> 0.001)
  
  model_performance(fit_binary)
  ...
  check_overdispersion(m.zip.TMB)
  check_zeroinflation(m.zip.TMB)
  
plot(mod2$fitted)
check_model(mod2)

mod2subset$predict.mod2<-predict(mod2, type="response")
mod2subset$presiduals.mod2<-resid(mod2, type="pearson")
p1<-ggplot(mod2subset,aes(predict.mod2, n))+geom_point()+
    geom_jitter()+geom_smooth( formula = y~ log(x), method ="glm")+
  xlab("Predicted")+ylab("Observed")
p1
p2<-ggplot(mod2subset, aes(predict.mod2, presiduals.mod2)) + 
    geom_point() +
    geom_jitter()+
    geom_abline(intercept=0,slope=0) +
    geom_smooth() + 
...
trtsNP_A$predict.b<-predict(fit_binary, type="response")
trtsNP_A$presiduals.b<-resid(fit_binary, type="pearson")
p1<-ggplot(trtsNP_A,aes(predict.b, binary))+geom_point()+
    geom_jitter()+geom_smooth( method ="glm", family = "binomial")+
  xlab("Predicted")+ylab("Observed")
p1
p2<-ggplot(trtsNP_A, aes(predict.b, presiduals.b)) + 
    geom_point() +
    geom_jitter()+
    geom_abline(intercept=0,slope=0) +
    geom_smooth() + 
...
ggplot(trtsNP_A, aes(y=binary, x=biomass_sum)) + geom_point() + 
...
$$Y_{it} \mid b_i \sim \mathrm{Bernoulli}(p_{it}) \nonumber \\$$
$$\log\left(\frac{p_{it}}{1-p_{it}}\right) = \beta_0 + b_{0i} + (\beta_1 X_{it} \nonumber \\$$
