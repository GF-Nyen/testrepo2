...
metadata$stand <- as.factor(metadata$stand)
...
regendata$row <- as.factor(regendata$row)
regendata$stand <- as.factor(regendata$stand)
```
...
regendata$seed_trt <- as.factor(regendata$seed_trt)
...
trtz <- c("U","U","U","S","S","S","S","U")
stands<- c(3,4,5,6,7,8,9,10)
AverageLiveBAperHA<- c(15.02291,8.62360,11.40782,13.34491,10.57843,14.65041,14.55944,12.90943)

badiff<- cbind.data.frame(trtz, stands, AverageLiveBAperHA)
badiff$trtz <- as.factor(badiff$trtz)
badiff$stands <- as.factor(badiff$stands)


m007 <- aov(AverageLiveBAperHA ~ trtz, data= badiff)
summary(m007)
m006 <- aov(AverageLiveBAperHA ~ stands, data= badiff)
summary(m006)
```
...
test5 <- aov(gap_fractionaverage ~ seed_trt, data= lighttest)
summary(test5)
...
```{r}
library(vegan)
trts$stand <- as.factor(trts$stand)
trts$seed_trt <- as.factor(trts$seed_trt)
...
adonis2(input_dat ~ seed_trt, #seed trt alone is .06188
        trts,
        permutations = 99999,
        method= "bray",
        sqrt.dist = F,
...
p77 <- ggplot(scrs) +
  ...
  #geom_segment(data= ef1.scrs,aes(x = 0, xend = NMDS1, y = 0, yend = NMDS2),
    ...
    #geom_segment(data= ef3.scrs,aes(x = 0, xend = NMDS1, y = 0, yend = NMDS2),
     #          arrow = arrow(length = unit(0.25, "cm")), colour = "red1") + #this is for ppfd_...
  geom_text(data = finalspp, aes(x = NMDS2, y = NMDS3, label = Species), # data = vector names    # HERE YOU CAN CHANGE THE AXIS 
            size = 4, color= "darkred") + theme_minimal() 
  #geom_text(data = ef1.scrs, aes(x = NMDS1, y = NMDS2, label = "stand", nudge_y = .25), # data = vector names 
   #         size = 5, color= "orange") + theme_minimal()
...
sp<- c("MUEX","SONU","SOSE","TEVI","ARBE")

SONU_plots <- data %>% summarise(stand, row, species_code, dried_weight_g) %>%
                      group_by(stand,row,species_code)  %>%
                      summarise(spp_biomass =sum(dried_weight_g)) %>%
                      group_by(stand, row, species_code) %>% summarise(spp_biomass = sum(spp_biomass)) %>%
                      filter(species_code == "SONU")
SOSE_plots <- data %>% summarise(stand, row, species_code, dried_weight_g) %>%
                      group_by(stand,row,species_code)  %>%
                      summarise(spp_biomass =sum(dried_weight_g)) %>%
                      group_by(stand, row, species_code) %>% summarise(spp_biomass = sum(spp_biomass)) %>%
                      filter(species_code == "SOSE")
TEVI_plots <- data %>% summarise(stand, row, species_code, dried_weight_g) %>%
                      group_by(stand,row,species_code)  %>%
                      summarise(spp_biomass =sum(dried_weight_g)) %>%
                      group_by(stand, row, species_code) %>% summarise(spp_biomass = sum(spp_biomass)) %>%
                      filter(species_code == "TEVI")
ARBE_plots <- data %>% summarise(stand, row, species_code, dried_weight_g) %>%
                      group_by(stand,row,species_code)  %>%
                      summarise(spp_biomass =sum(dried_weight_g)) %>%
                      group_by(stand, row, species_code) %>% summarise(spp_biomass = sum(spp_biomass)) %>%
                      filter(species_code == "ARBE")
MUEX_plots <- data %>% summarise(stand, row, species_code, dried_weight_g) %>%
                      group_by(stand,row,species_code)  %>%
                      summarise(spp_biomass =sum(dried_weight_g)) %>%
                      group_by(stand, row, species_code) %>% summarise(spp_biomass = sum(spp_biomass)) %>%
                      filter(species_code == "MUEX")
spp_plots <- bind_rows(ARBE_plots,TEVI_plots,SOSE_plots,SONU_plots,MUEX_plots)
...
# Stacked
ggplot(spp_plots, aes(fill=species_code, y=spp_biomass,  x = factor(stand, levels = c(3, 4, 5, 10, 6, 7, 8, 9)))) + 
    geom_bar(position="stack", stat="identity") +
         scale_x_discrete("stand", drop = FALSE) +
            scale_fill_manual("legend", values = c("ARBE" = "darkgray", "MUEX" = "orange", "SONU" = "skyblue", "SOSE" = "pink2", "TEVI"= "green3"))+
               labs(y="Species Biomass(g)", x= "Stand") +
               theme_minimal()
